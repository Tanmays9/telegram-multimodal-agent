...
# Node 1: Router
# Decides how to respond (text vs audio) based on conversation context
...
# Node 2: Generate Text Response
# Core logic to generate the bot's reply
...
# Node 3: Summarize Conversation
# Compresses history to stay within context window limits
def summarize_conversation_node(state: TelegramAgentState):
    ...
    # Delete all but the last 2 messages to free up space, keeping recent context
    delete_messages = [RemoveMessage(id=m.id) for m in state["messages"][:-2]]

    return {"summary": response.content, "messages": delete_messages}

# Node 4: Generate Final Response
# Handles converting text to audio if that was the chosen response type
